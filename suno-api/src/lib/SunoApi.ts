...
const logger = pino();
export const DEFAULT_MODEL = 'chirp-auk';
...
function parseCookieHeader(input: string): Record<string, string> {
  ...
  const normalized = String(input || '').replace(/\b(?:set-cookie|cookie)\s*:\s*/gi, ' ');
...
class SunoApi {
  ...
  constructor(cookies: string, captchaKey?: string) {
    this.captchaKey = String(captchaKey || process.env.TWOCAPTCHA_KEY || '').trim();
    this.solver = new Solver(this.captchaKey);
    this.userAgent = new UserAgent(/Macintosh/).random().toString(); // Usually Mac systems get less amount of CAPTCHAs
    this.cookies = parseCookieHeader(cookies);
  ...
  */
  ...
  public async getCaptcha(): Promise<string|null> {
    ...
    if (!this.captchaKey) {
      throw new Error('Suno requires hCaptcha for this generation. Configure a 2Captcha API Key in Airadio 模型配置, then retry.');
    }
...
export const sunoApi = async (cookie?: string, captchaKey?: string | null) => {
  ...
  const resolvedCaptchaKey = String(captchaKey || process.env.TWOCAPTCHA_KEY || '').trim();
  const cacheKey = `${resolvedCookie}\n${resolvedCaptchaKey}`;
  const cachedInstance = cache.get(cacheKey);
  if (cachedInstance)
    return cachedInstance;

  // If not, create a new instance and initialize it
  const instance = await new SunoApi(resolvedCookie, resolvedCaptchaKey).init();
