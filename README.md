...
## 技术原理：
**第一步，**在应用后台配置AI大模型（我配置的deepseek-v4-flash）,配置TTS语音模型（用于给文字内容配音，我使用的是当前免费的MiMo-V2.5-TTS），配置音乐APP的api（我只接了一个狗的，VIP扫码即可），再配置一些公共的免费接口（我配置了alapi.cn的每日早报和微博热搜接口）

**第二步，**在后台编辑好单个节目，并作为预设节目。

**第三步，**设定节目的播出时间，一键生成当天节目。其他没设置节目的时间段，使用AI自动调用音乐api生成歌曲连播来填充时段。

还有很多功能不详述了，比如自己搜索添加歌曲、排序，自定义AI配音的背景音乐、节目转场音效等等……

## 临时演示站点

[https://fm.aabb.live/](https://fm.aabb.live/)
